...
type RolePo struct {
	...
	Auth     model.List[string] `json:"auth" gorm:"type:json;comment:角色code列表" doc:"|d 权限ID |t []string"` //权限编码列表 eg [auth1,...]
}
...
type RoleAuthForm struct {
	...
	Auth model.List[string] `json:"auth" gorm:"type:json;comment:角色code列表" doc:"|d 权限列表 |t []string |e [auth1,auth2...]"` //权限编码列表 eg [auth1,auth2...]
}
